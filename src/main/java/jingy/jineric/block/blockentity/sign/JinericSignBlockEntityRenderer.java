...
public class JinericSignBlockEntityRenderer implements BlockEntityRenderer<JinericSignBlockEntity> {
   ...
   private final Map<JinericSignType, JinericSignBlockEntityRenderer.SignModel> typeToModel;
   private final TextRenderer textRenderer;

   public JinericSignBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
      this.typeToModel = (Map)SignType.stream()
              .collect(
                      ImmutableMap.toImmutableMap(
                              signType -> signType, signType -> new JinericSignBlockEntityRenderer.SignModel(ctx.getLayerModelPart(EntityModelLayers.createSign(signType)))
                      )
              );
      this.textRenderer = ctx.getTextRenderer();
   }

   public void render(JinericSignBlockEntity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
      BlockState blockState = signBlockEntity.getCachedState();
      matrixStack.push();
      float g = 0.6666667F;
      SignType signType = getSignType(blockState.getBlock());
      JinericSignBlockEntityRenderer.SignModel signModel = (JinericSignBlockEntityRenderer.SignModel)this.typeToModel.get(signType);
   ...
   private static boolean shouldRender(JinericSignBlockEntity sign, int signColor) {
   ...
   private static int getColor(JinericSignBlockEntity sign) {
   ...
   public static JinericSignBlockEntityRenderer.SignModel createSignModel(EntityModelLoader entityModelLoader, SignType type) {
      return new JinericSignBlockEntityRenderer.SignModel(entityModelLoader.getModelPart(EntityModelLayers.createSign(type)));
   }
